...
This post is a thorough explanation of my process for creating a model for the "Pump it Up" competition hosted by DrivenData. The model's classification rate is in the top 10% of all submissions according to the leaderboard at the time of this writing. 
...
After running the model with the optimal hyperparameters, we get an out-of-bag score or 0.8134. Although this is supposed to be an unbiased estimate of the classification rate, when we do cross-validation, we get a score of 0.8071, which is a lot closer to our test score 0.8059.
...
All of the code from this analysis can be found on my Github page: https://github.com/zlatankr/Projects/tree/master/Tanzania
